...
class artist3(artist):
	def __init__(self,dimensions,skateheight,fov,near,far):
	...
	def make_frustum(self):
		...
		y_far = tang * self._far
		x_far = y_far * self.ratio
		
		self.near = -self.near
		self.far = -self.far
		
		this.pm = matrix4( vector4(cosf,0,0,0),vector4(0,-cosf,0,0),vector4(0,0,sinf/nf,-(math.sin(1)/nf)*self.near),vector4(0,0,sinf,0) )
		this.vm = matrix4( vector4(99,0,0,self.dimensions.x),vector4(0,99,0,self.dimensions.y),vector4(0,0,1,0),vector4(0,0,0,1) ) # i dobnt know what the 99 is
		this.rm = self.pm * self.vm
